...
/* Who we help: the six HQA-D32 sectors as a typographic mosaic, closed by
   the house line (HQA-D16). The Verde claro band is gone (V3): the section is
   transparent and the atmosphere carries the colour. Photos (HQA-D31) can
   replace tiles later; typography is the honest default. */
export async function Ayudamos() {
  const t = await getTranslations("home.ayudamos");
  const giros = t.raw("giros") as string[];

  return (
    <section id="ayudamos" className="py-section">
